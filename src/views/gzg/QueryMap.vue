...
export default {
  ...
  methods: {
    ...
    initLayers() {
      ...
      window.MAP.addSource("stations_heatmap", {
        type: "raster",
        tiles: [
          "http://8.134.70.156:8181/geoserver/gpzi/wms?SERVICE=WMS&VERSION=1.1.1&REQUEST=GetMap&FORMAT=image/png&TRANSPARENT=TRUE&STYLES&LAYERS=gpzi:stations_heatmap&exceptions=application/vnd.ogc.se_inimage&SRS=EPSG:3857&WIDTH=769&HEIGHT=750&BBOX={bbox-epsg-3857}",
        ],
        tileSize: 512,
      });
      window.MAP.addLayer({
    ...
    loadWFS(features) {
      ...
      this.axios
        .get("http://localhost:8080/geoserver/wfs", {
          params: {
            service: "WFS",
            version: "1.1.0",
            request: "GetFeature",
            typeName: "	gzg:gzg-stations",
        ...
        .then((res) => {
          ...
          console.log(ids.length,'afasdfsfsdf');
          if(ids.length == 0){
            _this.$message.error('查询超出广州港范围');
            return;
          }else{
  ...
  destroyed() {
    removeLayers(window.MAP, ["stations_heatmap"]);
    deleteEvt(window.MAP);
  },
};
</script>
